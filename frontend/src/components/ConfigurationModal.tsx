...
const ConfigurationModal: React.FC<ConfigurationModalProps> = ({ isOpen, setIsOpen, onConfigurationComplete, initialConfig }) => {
	...
	const [testStatus, setTestStatus] = useState<{
		ollama?: 'Successfully tested!' | 'Testing...' | 'Test failed. Please ensure configurations are correct and try again.';
		included?: string;
	}>({});
	...
	const aiProviderOptions = [
		{
			name: 'included',
			title: 'Included',
			description: 'Use a provided AI model (Llama 3.3 70B) to try out LLM Client without additional configuration.',
			iconRenderFunction: (customClass: string) => <Cloud className={customClass} />,
			configurationSteps: [],
			verificationSteps: [
				{
					...
					buttonOnClick: async () => {
						try {
							...
							setTestStatus({
								...testStatus,
								included: data.credits ? 'Successfully tested!' : 'Test failed. Please try again.',
							});
						} catch (error) {
							setTestStatus({
								...testStatus,
