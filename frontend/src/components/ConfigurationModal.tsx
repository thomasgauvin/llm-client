...
const ConfigurationModal: React.FC<ConfigurationModalProps> = ({
  ...
  const [selectedOption, setSelectedOption] = useState<
    "ollama" | "included" | "cloudflare" | "custom" | null
  >(() => {
  ...
  const [cloudflareUrl] = useState<string>(() => {
    return config.type === "cloudflare"
      ? config.config?.url || "https://localhost:8443"
  ...
  // Verification states
  const [ollamaOriginVerified, setOllamaOriginVerified] =
    useState<boolean>(false);
  const [ollamaModelsVerified, setOllamaModelsVerified] =
    useState<boolean>(false);
  const [includedOriginVerified] = useState<boolean>(true);
  ...
  useEffect(() => {
    if (
      ...
      fetch(
        `${cloudflareUrl}/client/v4/accounts/${cloudflareAccountId}/ai/models/search`,
        {
          headers: {
  ...
  const isCurrentStepVerified = () => {
    ...
    switch (selectedOption) {
      case "included":
        return includedOriginVerified;
      case "ollama":
        return ollamaOriginVerified && ollamaModelsVerified && selectedModel;
      case "cloudflare":
        return (
          cloudflareAccessVerified && cloudflareToken && cloudflareAccountId
        );
      case "custom":
  ...
  const aiProviderOptions = [
    {
      ...
      title: "Included",
      description:
        "Use a provided AI model (Llama 3.3 70B) to try out LLM Client without additional configuration.",
      iconRenderFunction: (customClass: string) => (
        <Cloud className={customClass} />
      ),
      configurationSteps: [],
      verificationSteps: [
        {
          label: "Verify API Access",
          buttonRenderFunction: (onClick: () => void) => (
          ...
          buttonOnClick: async () => {
            try {
              ...
              setTestStatus({
                ...testStatus,
                included: "Test failed. Please try again.",
              });
            }
          },
          status: testStatus.included,
        },
      ],
    },
    {
      ...
      configurationSteps: [
        {
          ...
          input: {
            value: `launchctl setenv OLLAMA_ORIGINS "${window.location.href.substring(
              0,
              window.location.href.length - 1
            )}"`,
            disabled: true,
          ...
          postInputText: (
            <>
              <ReactMarkdown className="dark:text-zinc-300">
                After executing this command, restart Ollama. Refer to the
                [Ollama
...
caddy reverse-proxy --from https://<YOUR_LOCAL_IP>:8443 --to http://localhost:11434`,
        ...
        },
        {
          ...
          postInputText: (
            <>
              Accept the self-signed certificate in your browser if you haven't
              already.
              <a
                href={ollamaUrl}
        ...
        },
        {
          ...
          postInputText:
            ollamaModels.length === 0 ? (
              <>
                No models found. Make sure Ollama is running and the reverse
  ...
  const testCurrentStep = () => {
    if (selectedOption) {
      const currentStep = aiProviderOptions.find(
  ...
  const handleOptionSelect = (option: string) => {
    if (option != "ollama" && option != "included" && option != "cloudflare") {
      return;
    }
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <div className="space-y-4">
            <Dialog.Title className="text-xl font-medium mb-4 text-zinc-900 dark:text-white mx-6">
              ...
              {aiProviderOptions.map((option) => (
                ...
                >
                  <div className="text-zinc-900 dark:text-white">
                    {option.iconRenderFunction("w-4 h-4")}
                  </div>
                      ...
                      {platform === "web" && selectedOption === "cloudflare" ? (
                        <div className="space-y-4 text-base mx-6">
                          ...
                          <p className="text-red-600">
                            The Cloudflare REST API can only be accessed through
                            our desktop or mobile applications.
                          </p>
                        ...
                        <div key={option.name} className="space-y-4">
                          ...
                          <div className="space-y-2 mx-6">
                            {option.configurationSteps.length > 0 && (
                          ...
                          {option.verificationSteps.length > 0 && (
