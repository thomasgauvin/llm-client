...
const ChatApp: React.FC = () => {
  const [aiConfiguration, setAiConfiguration] = useAiConfiguration(
    undefined
  ) as [
  ...
  const [ollamaApiBaseUrl, setOllamaApiBaseUrl] = useState<string>(
    localStorage.getItem("ollama_api_base_url") || "http://localhost:11434"
  );
  const [ollamaApiUrl, setOllamaApiUrl] = useState<string>(
    `${ollamaApiBaseUrl}/api/generate`
  );
  ...
  const deleteConversation = async (id: number) => {
    ...
    setConversationId(
      conversations.length > 1 ? conversations[0].id || null : null
    );
  };
  ...
  return (
    <div className="flex flex-col h-screen w-screen overflow-clip bg-white dark:bg-zinc-800">
      <ChatNavbar
